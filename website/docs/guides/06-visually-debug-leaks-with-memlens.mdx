...
**Note:** If the target page is a React app serving minified
JS code, the detached DOM element’s retainer path will also appear
minified when you search for the memory ID in Chrome DevTools.

## Using MemLens with MemLab

MemLens can also run during MemLab's headful mode to display
leak outlines while a test scenario executes:
