...
**Part 1**: The browser interaction breadcrumb shows the
interactions (navigations) that `memlab` performed as specified in the scenario file.

  * **`page-load[6.5MB](baseline)[s1]`** - The JavaScript heap size was
    `6.5MB` on initial page load. The `baseline` heap snapshot is saved as
  ...
  * **`revert[7MB](final)[s3]`** - The heap reached `7MB`
...
**Part 2**: Overall summary of the leak trace
  ...
  * **`Retained size`** - The aggregated retained size of the leaked object
    cluster is `143.3KB` (memory leaks are grouped together based on the
    similarity of their retainer traces).
...
:::
  ...
  * `0` - This shows that a detached `HTMLDIVElement` (i.e., a DOM element that is no longer connected to the DOM tree) is stored as the first element of the `leakedObjects` array. (Since showing all 1024 leak traces would be overwhelming,
    memlab prints only one representative trace — property 0 instead of properties 0 through 1023.)
