...
run API exported by `@memlab/api`.
```typescript
...
* **Parameters**:
  * `snapshot`: <code>[IHeapSnapshot](IHeapSnapshot.md)</code> | the final heap
     snapshot taken after all browser interactions are done.
     Check out [IHeapSnapshot](IHeapSnapshot.md) for more APIs that queries the
     heap snapshot.
...
* **Parameters**:
  * `node`: <code>[IHeapNode](IHeapNode.md)</code> | the heap object
...
For concrete examples, check out [leakFilter](#leakfilter).

* **Parameters**:
  * `edge` : <code>[IHeapEdge](IHeapEdge.md)</code> | the reference (edge)
     that is considered for calcualting the retainer trace
  * `snapshot`: <code>[IHeapSnapshot](IHeapSnapshot.md)</code> | the heap snapshot
...
if you have to (we recommend using [cookies](#cookies))
or to prepare data before the [action](#action) call.
...
the string value of the initial url
* **Examples**:
```typescript
const scenario = {
  url: () => 'https://www.npmjs.com/',
